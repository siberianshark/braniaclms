...
class TestDocsitePage(TestCase):
    def test_page_open(self):
        path = reverse('mainapp:docs')
        result = self.client.get(path)
        self.assertEqual(result.status_code, HTTPStatus.OK)
...
class TestMainPage(TestCase):
    ...
    def setUp(self):
        ...
        self.user_admin = authapp_models.CustomUser.objects.get(username="django")
...
#     def test_page_open(self):
#         news_obj = authapp_models.CustomUser.objects.first()
#         path = reverse("authapp:profile_edit", args=[news_obj])
#         result = self.client_with_auth.get(path)
#         self.assertEqual(result.status_code, HTTPStatus.OK)
...
class TestNewsSelenium(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        button_enter = WebDriverWait(self.selenium, 10).until(
    ...
    def test_pick_color(self):
        path = f"{self.live_server_url}{reverse('mainapp:mainapp')}"
        self.selenium.get(path)
        navbar_el = WebDriverWait(self.selenium, 10).until(EC.visibility_of_element_located((By.CLASS_NAME, "navbar")))
        try:
            self.assertEqual(
